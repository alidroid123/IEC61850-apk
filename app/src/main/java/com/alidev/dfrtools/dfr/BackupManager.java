 ...
 * and relay templates - into one JSON file, so it survives a phone swap or can be shared to a
 * teammate working on the same substations. Operates directly on the same raw JSON blobs the
 * respective managers persist (dfr_prefs/device_list, monitoring_prefs/monitored_nodes,
 * relay_templates_prefs/templates) rather than through their model classes, so a backup from an
 * older app version still merges whatever fields are present instead of failing to parse.
 */
public class BackupManager {
    private static final int BACKUP_FORMAT_VERSION = 1;

    public static class ImportResult {
        public int devicesAdded, nodesAdded, templatesAdded;
    }
